...
**Fonte de verdade de stack:** `stack.spec.md` + `decisions.md`. Itens abaixo que ainda não estão “travados” no `stack.spec.md` entram com **ADR** quando forem decididos para implementação.
...
|------------------|-------------------------|--------|
| **React** | Next.js usa React como base da UI | ✅ Já na stack (`stack.spec.md`) |
...
1. **Upload** de documento (PDF/imagem) ligado a **estudante** ou **matrícula** (escopo mínimo em `docs/specs/ai/ingestion.spec.md` quando existir).
...
2. **Fechar specs finos:** catálogo, finance, API/errors (matriz em `specs/README.md`).
...
## O que ainda **não** está no `stack.spec.md`

Para implementar de fato: atualizar `stack.spec.md` e `decisions.md` com ADRs curtos para, no mínimo:
